...
//  through here — it's rendered via YouTubePlayerKit's own YouTubePlayerView
//  instead (see VideoPlayerView), since that already wraps YouTube's
//  official IFrame Player API with a maintained, ToS-compliant integration.
//
    ...
    final class Coordinator: NSObject, WKNavigationDelegate {
        ...
        func webView(_ webView: WKWebView, didFinish navigation: WKNavigation!) {
            ...
            DispatchQueue.main.asyncAfter(deadline: .now() + 0.9) { [weak self, weak webView] in
                ...
                // A real page (or a blank/login-walled one) is judged by its
                // text content, but a video embed plugin page (Facebook's
                // video.php, Vimeo/Dailymotion/Twitch's own embed pages) is
                // legitimately almost text-free — it's just a player — so a
                // <video>/<iframe> in the DOM counts as rendered content too.
                webView.evaluateJavaScript(
                    "(document.body ? document.body.innerText.trim().length : 0) + (document.querySelector('video, iframe') ? 20 : 0)"
                ) { result, _ in
